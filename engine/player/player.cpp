...
void player_t::init_base_stats()
{
  ...
  if ( !is_enemy() )
  {
    base.stats.attribute[ ATTR_STRENGTH ] =
      dbc->race_base( race ).strength + dbc->attribute_base( type, level() ).strength;
    base.stats.attribute[ ATTR_AGILITY ] =
      dbc->race_base( race ).agility + dbc->attribute_base( type, level() ).agility;
    base.stats.attribute[ ATTR_STAMINA ] =
      dbc->race_base( race ).stamina + dbc->attribute_base( type, level() ).stamina;
    base.stats.attribute[ ATTR_INTELLECT ] =
      dbc->race_base( race ).intellect + dbc->attribute_base( type, level() ).intellect;
    base.stats.attribute[ ATTR_SPIRIT ] =
      dbc->race_base( race ).spirit + dbc->attribute_base( type, level() ).spirit;
    ...
    {
      // Rating Multipliers
      base.rating_multiplier[ rating ] =
        get_passive_player_value( base.rating_multiplier[ rating ], "rating_multiplier", rating );
    }
    ...
    // school based multipliers
    for ( auto school = SCHOOL_NONE; school < SCHOOL_MAX_PRIMARY; ++school )
    {
      base.damage_multiplier[ school ] =
        get_passive_player_value( base.damage_multiplier[ school ], "damage_multiplier", school );
      base.damage_multiplier[ school ] *= get_passive_player_value( 1.0, "damage_multiplier", -1 );

      base.crit_damage_multiplier[ school ] =
        get_passive_player_value( base.crit_damage_multiplier[ school ], "crit_damage_multiplier", school );
    ...
    base.pet_damage_multiplier =
      get_passive_player_value( base.pet_damage_multiplier, "pet_damage_multiplier" );
    base.guardian_damage_multiplier =
      get_passive_player_value( base.guardian_damage_multiplier, "guardian_damage_multiplier" );
...
sample_data_helper_t* player_t::get_sample_data( util::string_view name )
{
  ...
  if ( !sd )
  {
    sd = new sample_data_helper_t( name, player_collected_data_t::generic_container_type( this, 3 ) );
...
struct pool_resource_t : public action_t
{
  ...
  bool ready() override
  {
    ...
    if ( next_action )
    {
      ...
      double extra = amount_expr ? amount_expr->eval() : 0.0;

      // must at least have more current resources than the extra amount
      if ( current >= extra )
      {
        // check if we can cast next ability after accounting for the extra amount required
        player->resources.current[ resource ] -= extra;
        bool next_is_ready = next_action->action_ready();
        player->resources.current[ resource ] = current;

        // if next action is ready, we'll have enough resource remaining after cost so we don't need to pool
        if ( next_is_ready )
          return false;
        ...
        if ( force_wait )
        {
          assert( next_action->current_resource() == resource &&
                  "Cannot pool to force_wait with mismatched resources." );

          double next_cost = next_action->cost();
          double diff = next_cost + extra - current;
      ...
      {
        // not enough resources to cover the extra, even if next action has 0 cost
        return false;
      }
    }
...
std::array<double, 3> player_t::get_passive_value( const spelleffect_data_t& eff, std::string_view field ) const
{
  assert( !is_pet() || get_owner_or_self() != this );
  if ( is_pet() )
...
std::vector<const spell_data_t*> player_t::spells_affected_by_passive( const spelleffect_data_t& eff, bool& prop ) const
{
  ...
  for ( const auto& list : registered_affected_spell_list_ )
  {
    if ( list.first == eff.id() )
    {
...
std::pair<player_t::modified_value_t, const player_t::modified_value_t&> player_t::add_passive_effect_modifier(
  std::vector<player_t::modified_value_t>& modifiers, int id, int field_id, double orig_val, double flat_val,
  double pct_val )
{
  auto it = range::find_if( modifiers, [ id, field_id ]( const auto& mod ) {
    return mod.id == id && mod.field_id == field_id;
  } );
  if ( it == modifiers.end() )  // cache original value if new modification
  {
    modifiers.emplace_back( id, field_id, orig_val );
    it = std::prev( modifiers.end() );
  }

  auto prev = *it;  // make a copy
...
bool player_t::register_passive_effect( const spelleffect_data_t& modifying_eff, bool remove )
{
  ...
  if ( affected_spells.empty() )
  {
    ...
    switch ( m_it->type )
    {
      case MISC_LIST_ATTR:
      {
        ...
        if ( misc_type == -2 )
          attr_list = { ATTR_STRENGTH, ATTR_AGILITY, ATTR_INTELLECT };
        else if ( misc_type == -1 )
          attr_list = { ATTR_STRENGTH, ATTR_AGILITY, ATTR_STAMINA, ATTR_INTELLECT, ATTR_SPIRIT };
        else
          attr_list.push_back( static_cast<attribute_e>( misc_type + 1 ) );
  ...
  for ( auto spell_ : affected_spells )
  {
    ...
    else if ( property )
    {
      ...
      {
        // effect modifier
        case P_EFFECT_1: eff_idx =  1; break;
        case P_EFFECT_2: eff_idx =  2; break;
        case P_EFFECT_3: eff_idx =  3; break;
        case P_EFFECT_4: eff_idx =  4; break;
    ...
    // TODO: implement for _pct_cost
    else if ( !pow_field.empty() )  // modify spellpower_data_t
    {
      // populate selected power(s)
      std::vector<const spellpower_data_t*> pow_list;
      for ( size_t i = 0; i < spell->power_count(); i++ )
      {
        if ( pow_idx_bit & ( 1U << i ) )
        {
          auto pow = spell->powerN( i + 1 );
          auto id = pow.id();
          if ( !id )
            continue;
    ...
    else if ( !eff_field.empty() )  // modify spelleffect_data_t
    {
    ...
    else if ( eff_idx != 0 )  // modify another effect
    {
      ...
      if ( eff_idx == -1 )
        ...
        eff_list.push_back( &spell->effectN( eff_idx ) );
...
void player_t::parse_all_passive_talents()
{
  ...
  {
    if ( !rank || tree == talent_tree::SELECTION )
      continue;
