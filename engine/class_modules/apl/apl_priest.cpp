...
void shadow( player_t* p )
{
  ...
  main->add_action( "variable,name=dots_up,op=set,value=(active_dot.vampiric_touch=active_enemies|action.tentacle_slam.in_flight)&(active_dot.shadow_word_pain>=active_dot.vampiric_touch|action.tentacle_slam.in_flight&talent.misery),if=active_enemies<3" );
  main->add_action( "shadow_word_pain,if=action.tentacle_slam.in_flight&!dot.shadow_word_pain.ticking&!talent.misery&active_enemies<3", "Apply SWP after opener slam when not talented into Misery so we can enter cooldowns" );
  ...
  main->add_action( "tentacle_slam,target_if=min:dot.vampiric_touch.remains,if=(dot.vampiric_touch.refreshable&time>1)|cooldown.tentacle_slam.full_recharge_time<=gcd.max*2", "Use Tentacle Slam to prevent capping charges or to refresh Vampiric Touch" );
  ...
  main->add_action( "tentacle_slam,target_if=min:dot.vampiric_touch.remains,if=(talent.void_apparitions|talent.maddening_tentacles)&time>1&(raid_event.adds.in>30|raid_event.adds.in>5&cooldown.tentacle_slam.full_recharge_time<=gcd.max*2)", "Use Tentacle Slam for Void Apparitions or Maddening Tentacles value, holding for adds if needed" );
